...
| **W3** | The store re-derives and persists `Run.record_completeness_status` inside every write that can change it, using the rules of `store/completeness.py`; a reader never computes completeness (FR-010). Since T298 that derivation can answer `in_flight` as well as `complete`/`has_gaps`/`unknown`: `turn_gaps` exempts the in-flight turn of an actively-playing run (correctly — FR-007 puts the quicksave before the `TurnCycle`), and `in_flight` is the price of that exemption said out loud, so a run that halted without its lifecycle state being transitioned cannot be reported `complete` |
...
| # | Requirement |
|---|---|
| **T1** | `metric_series` excludes, by the store's rule, any run whose record carries game turns that did not advance (`game_turn_did_not_advance`, added 2026-09-21 per research R6 — see data-model.md §3.5), whose completeness is **anything other than `complete`** (amended 2026-09-22 per T298 — it read `has_gaps`/`unknown`, an enumeration of the bad values that admitted every state it had not been taught, including `in_flight`, which T298 added for a run whose in-flight turn is unrecorded; `in_flight` is refused under its own reason, `record_in_flight`), whose comparability is `not_comparable`, or which is `visually_degraded` unless the query opted in; every excluded run is named with its reason and its gaps. `trend_exclusion(run_id, *, include_visually_degraded=False)` publishes that same verdict for one run without requesting a series, including `no_such_run` for an id the store does not hold (FR-019, SC-007) |
...
| **T3** | A non-terminal run's series carries `in_progress=True` (FR-021) — when it gets a series at all: since T298 a non-terminal run whose completeness derives `in_flight` (a quicksave with no `TurnCycle` behind it) is excluded under `record_in_flight` rather than served, so T3 applies to a live run during the window its record is whole, which is every moment between a turn's `TurnCycle` landing and the next turn's quicksave. See spec.md US3/AC3 for why the mid-turn case cannot be admitted |
