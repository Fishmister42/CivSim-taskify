...
## Stage 3 — 14:24 EDT → 16:10 EDT (Linux live lane, third stage)

| Block | Run | Game turns | Provider | Applied / issued | Stalls & fixes | Frames | Cost |
|---|---|---|---|---|---|---|---|
| 20 | run-5bd1a86b | 49 → 53 (5 harness turns, every one ended on the 8-step no-progress backstop, each backstop end turn game-confirmed) | Sonnet 5 | **0 / 40**: `prompts.era_transition` with target "Free Inquiry" (four spellings) ×40, every one refused `unavailable_to_human_now` because the text probe reported `world`, no blocking prompt. The frame shows **"Make Your Dedication For The Classical Era"** (Dark Age, four dedications + Confirm) — an unmapped chooser distinct from the `EraReviewPopup` card block 19 acknowledged. **40 of 40 images delivered**; the picture was the only channel that carried the panel, and the model read it correctly and consistently. Yields recorded every turn (science 4.05, culture 2.25, gold 4.5, faith 4.5; gold 182 → 200) | dedication chooser unmapped (not fixed this stage); `city.available_productions` is `[]` in every observation while `production_queue` is `[]`, so `cities.set_production`'s predicate can never hold — the body never fills the list (recorded, not fixed). Deadlock fix `c795039` landed headless, **not live-verified** (no save-blocking prompt occurred). No first meeting occurred. | keyframe_0 (panel), keyframe_3 (end) | $1.27 |
...
## Stage 4 (18:00–20:05 EDT) — blocks 21–25, goal runs 01–05, the first game-over and recovery

| block/goal | run | game turns | provider | result |
|---|---|---|---|---|
| 21 | run-b4531bbe | 54 → 57 | Sonnet 5 | civic ack applied on the 9th try; research set; paused on unmapped `WorldCongressIntro` |
| 22 | run-0feb5d55 | 57 → 58 | Sonnet 5 (HEAD 3aad0c8, first under availability rendering) | 16× `ai_diplomatic_approach` "Goodbye" rejected — Exit needs CloseSession |
| 23 | run-c3176fb2 | — | stochastic/coverage | paused in 2.5 s: ParityViolation on `production_options[].cost` (working-tree fix, key name) |
| 24 | run-fbd6c25e | 58 → 59 | stochastic/coverage (worktree 1d0b372) | **first populated production list**; research + save applied |
| 25 | run-729e62bc | — | stochastic/coverage | **GAME OVER**: Persia defeated at t59; recorded as SaveVerificationError (finding → f31fb5b) |
| recovery | — | reload to 42 | operator | post-defeat menu refused Network.LoadGame ×3; exit modal ignored synthetic input; killed at menu (data-safe); relaunch; fresh menu loaded t42 in 38.0 s; ~17 min total |
| goal-01/02 | — | — | Sonnet 5 | stale prerequisite marker; then `resolve_provider(policy)` TypeError (fixed ed4c793) |
| goal-03 | run-1091122b | 42 | Sonnet 5 | CredentialResolutionError — secrets.yaml absent in the worktree (copied, gitignored) |
| goal-04 builder chain | run-26d265f6 | 42 | Sonnet 5 (ba9ac6e) | **`prompts.era_transition` + `prompts.era_dedication` applied (new)**; `set_production` UNIT_BUILDER rejected 9× |
| goals change_research / send_delegation / save_named_game | run-d0933ca8 … | 42 | Sonnet 5 | 0 applied each: first-meeting greeting statement answer fails verification (16/16 each); cleared by operator CloseSession |
| goal-05 builder chain | run-02168773 | 42 → 53 | Sonnet 5 (59af4a2, set_production fix) | **`prompts.natural_disaster` applied (new)**; `set_production` rejected **80/80** with the fix; result.json absent (outer timeout cut the driver) |
