...
| 5 | `run-8bc17e7e…` | **paused** at turn 3 (`UnknownScreenEncountered`, game 15→17) | 16 | $0.294 | `units.move_to {"target": 131073}` ×16 — still the unit id, not a plot | correctly refused on `target in unit.reachable_plots`; then Code of Laws completed, `TechCivicCompletedPopup` came up, no catalog screen maps it → the run stalled visibly (FR-010) instead of pressing anything. The client was left on that popup. |
