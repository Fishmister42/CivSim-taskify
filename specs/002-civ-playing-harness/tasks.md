...
- [ ] T212 Produce a real `SupportProbeResult` in `src/civsim_harness/host/detect.py` (an R19 per-platform probe: verify the quicksave path, report the R6 capture-hygiene spike result) and wire it through `run/composition.py`'s `build_runner_dependencies` and `operator/cli.py`'s `default_runner_factory` per **FR-054** (missing) — nothing in `src/` constructs anything but `UNPROBED` today, so `evaluate_host_gate` refuses **every** run on **every** host, including the validated Linux one
...
- [ ] T214 Close the `NexusClient` when a run reaches a terminal state, per **FR-006** (partial): `run/composition.py` opens one client per `prepare_run` and nothing ever closes it, and the client accepts a single tuner connection at a time — so a second `run start` in the same process cannot connect
- [ ] T215 Implement `Runner.resume_from` in `src/civsim_harness/run/runner.py` per **FR-004** (missing): it still raises `HarnessError("not implemented in this wave")`, and `civsim run resume-from` now reaches it for the first time rather than failing earlier at "no runner is configured"
- [ ] T216 Resolve victory/defeat in `run/composition.py`'s `_evaluate_stop_facts` per **FR-005** (partial): it reports only `current_turn`, so a run configured with a `game_outcome` stop condition can never resolve. Needs a catalog declaration that reads the game's own outcome — none exists in `catalogs/` today
...
- [ ] T219 Resolve `RunConfiguration.guidance_set_id` through `config/guidance.py` and pass the loaded `GuidanceSet` into `DecisionLoopContext` in `run/composition.py` per **FR-021** (missing) — `guidance` is hard-coded `None`, so configured guidance never reaches the agent
- [ ] T220 Set `Run.capture_path` from the host's actual resolved capture mechanism in `run/composition.py` per **FR-050**, **SC-013** (partial) — it is hard-coded to `CapturePath.NONE`, so a run on a host with a working capture path is recorded as having none
- [ ] T221 Supply a real `camera_state_provider` to `DecisionLoopContext` in `run/composition.py` per **FR-026** (partial) — it is left at its `dict` default, so every capture records an empty camera state and no view's declared camera requirements are ever satisfied
- [ ] T222 Report real tuner and client health from `run/composition.py`'s `connection_health()` per **FR-053** (partial) — both are hard-coded `"unknown"`, so `run status` cannot distinguish a live client from a dead one
