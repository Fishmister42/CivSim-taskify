...
| `lua_arguments` | actions (optional) | 2026-09-22: the ordered positional arguments this declaration's Lua function takes. Each entry is `target` (the agent's own `parameters.target`) or `parameters.<name>` (a named value the agent also supplies) — there is no third form, no literal constants and no optional arguments with defaults. Every declared argument is **required**: a decision missing one is refused at dispatch with `missing_required_argument` and never dispatched with fewer. Must include `target` whenever `target_kind` is set and is not `none`. Omit the field entirely for an action whose Lua takes the single positional argument `act/executor.py`'s original convention produces — those behave exactly as before. Rendered to the agent as a ` -- also send: …` tail (`agent/context.py`), derived from this field so the listing cannot drift from it |
