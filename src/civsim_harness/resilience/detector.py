...
a single bounded round-trip (heartbeat), a single bounded call
(per-operation bounds), or a single probe result (screen identity) --
...
TIMEOUT_S`, the per-kind bounds in `DEFAULT_OPERATION_BOUNDS_S`), not by
this module imposing a wall-clock deadline of its own.
...
async def check_heartbeat(
    ...
    *,
    ...
    """`hang_detected` when one heartbeat round-trip does not complete in time.

    research R12 names both "hang, deadlock" and "dropped tuner connection"
    as this signal's catches, so both non-OK `HeartbeatOutcome` values map
    to `hang_detected` here -- the outcome itself is preserved in `detail`
    for anyone reading the record back.
    """
...
class DetectionAggregator:
    ...
    one heartbeat round-trip, one screen probe. It does not loop, sleep, or
    retry -- any polling cadence belongs to the caller (the run
