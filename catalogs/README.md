...
| 2026-09-22 (`2026.09.20`) | `game.screen_state` — new fields `screen_probe_reason`, `screen_probe_error`, `popup_stack_depth`, `popup_stack_ids` | **The allowlist stops being read as a detector.** `recognized: true` meant two different things — "I identified what is on screen" and "nothing on `CIVSIM_SCREEN_WATCHLIST` is open" — and production took the second and reported the first **three times**: `WorldCongressIntro` (turn 57), `EndGameMenu` (a full-screen DEFEAT modal, turn 67) and `HistoricMoments` (the "Era Makes History" card, 2026-09-22 ~17:15, where `turn.end_turn` would have been authorised against it). Each was patched by appending one name; the game exposes ~140 Lua contexts and the watchlist named 33. The probe now asks **the engine** which contexts are showing — `UIManager:GetPopupStack()`, shipped Firaxis API read at `base/assets/ui/utilities/tunerutilities.lua:187-192`, whose entries carry `.ID`/`.Priority`/`.Flags` — **before** consulting any list of ours, and all three confirmed instances are `UIManager:QueuePopup` popups (`worldcongressintro.lua:43`, `endgame/endgamemenu.lua:755`, `historicmoments.lua:430`) so one call covers them and every other queued modal with no list in the loop. A stack entry the probe cannot name, that its own context reports **showing**, is `screen: unknown` and the run stalls (FR-049); a queued entry demonstrably **hidden** does not stall, because something above it is showing and that something is on the stack too. `popup_stack_depth` is the **evidence the `world` answer now rests on** and is absent exactly when the stack could not be read — in which case `recognized` is false and `screen_probe_reason` says which branch fired, because absence and unobservability must not share a representation. The watchlist sweep is unchanged and still names every screen it knows, including the two full-screen views that are not UIManager popups (`GreatWorkShowcase`, `NaturalDisasterPopup`). **UNVERIFIED LIVE:** that `UIManager:GetPopupStack` is bound in the `InGame` context; on a build where it is not, every probe stalls with `screen_probe_reason = "popup_stack_unreadable"` — loud and one line to diagnose, rather than silent and confident. |
